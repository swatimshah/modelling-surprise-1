...
# Create the model
model=Sequential()
model.add(Conv1D(filters=60, kernel_size=10, kernel_regularizer=L2(0.001), bias_regularizer=L2(0.001), activity_regularizer = L2(0.001), kernel_constraint=min_max_norm(min_value=-2, max_value=2), padding='valid', activation='relu', strides=1, input_shape=(20, 1)))
model.add(Conv1D(filters=60, kernel_size=10, kernel_regularizer=L2(0.001), bias_regularizer=L2(0.001), activity_regularizer = L2(0.001), kernel_constraint=min_max_norm(min_value=-2, max_value=2), padding='valid', activation='relu', strides=1))
...
mc = ModelCheckpoint('E:\\recording-car\\best_model.h5', monitor='val_accuracy', mode='max', verbose=1, save_best_only=True)
